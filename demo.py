...
class Demo:
    ...
    def format_output(self, batch, output,img, original_shape ):
        ...
        for index in range(batch['image'].size(0)):
            ...
            if self.args['polygon']:
                ...
                with open(result_file_path, 'wt') as res:
                    regions = []
                    fname = os.path.abspath(result_file_path)
                    ...
                    for i in range(boxes.shape[0]):
                        ...
                        char_img = get_box_img(box,img)
                        d = detect_symbol(char_img)
                        if(d):
                            region = add_JSON_detect(d,box)
                            regions.append(region)
                            detect_count = detect_count+1

                    result = to_JSON(regions,fname,file_size)
                    json_detect = json.dumps(result, indent=4, sort_keys=True)
                    with open('boxes.json', 'w') as outfile:
                        json.dump(result, outfile)

                    print("detection rate: "+ str(detect_count/boxes.shape[0]))
    ...
    def inference(self, image_path, visualize=False):
        ...
        with torch.no_grad():
            ...
            self.format_output(batch, output,original_image , original_shape )
